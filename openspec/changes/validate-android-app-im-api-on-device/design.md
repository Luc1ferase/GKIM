...
The repository now has two pieces that have not yet met in a real product loop: a Rust IM backend with verified session/bootstrap/history/WebSocket behavior, and an Android client whose IM surface still runs on `InMemoryMessagingRepository`, `https://api.example.com/`, and `wss://example.com/realtime`. The user wants full API validation in real app usage on a physical device, which means the change must cover both runtime wiring and a repeatable device-validation path rather than only adding more unit tests.

There is also an environment constraint that matters immediately: the deployed backend currently listens on `127.0.0.1:18080` on the Ubuntu server. A physical Android device therefore cannot hit the backend directly unless we either expose a mobile-reachable gateway or forward traffic through a developer-controlled path. The design should prefer the smallest safe validation path first, keep database and server secrets out of the APK, and preserve the repository rule that each implementation slice must be verified, reviewed, scored, evidenced, and pushed independently.
...
- Let a physical Android device exercise the backend WebSocket message flow, including send, receive, delivered, read, reconnect, and visible failure states.
- Add a repeatable runtime configuration and validation workflow so operators can point the app at a forwarded or deployed IM backend without rebuilding the APK or embedding secrets.
...
### 2. Use a developer-controlled forwarding path for first-pass physical-device validation
The primary validation path will use a host-side SSH tunnel from the developer machine to `124.222.15.128:18080`, plus `adb reverse` to expose that forwarded port to a USB-connected Android device. The app will therefore target a loopback-style HTTP/WebSocket endpoint on the device during validation without requiring the backend service to stop binding to server-localhost.

Why this decision:
- It works with the backend's current bind address and avoids opening new public ingress just to validate the app.
- It keeps secrets and operational control on the developer machine instead of teaching the APK about server internals.
- It is fast to iterate on and well-suited to real-device debugging sessions.

Alternatives considered:
- Rebinding the backend directly to a public interface: rejected for the first validation change because it increases deployment risk and security surface.
- Standing up a public reverse proxy/TLS gateway first: viable later, but rejected for the initial device-validation loop because it adds infrastructure scope before the app contract is proven.
...
The app will surface connection/bootstrap/session failures in a visible debug-friendly manner, and the repository docs/tasks will define an evidence workflow that combines on-device steps, adb/logcat capture, and backend-side checks. Task slices will end with proof that a real device can run the targeted API subset.
...
- [ADB reverse requires a developer-connected Android device and host machine] → Document it as the primary debug path for this change and keep a future reverse-proxy/public-gateway option open.
...
4. Add automated verification around DTO mapping, repository reconciliation, and device-debug UI/state behavior.
5. Add the adb/SSH/device validation guide, run the real-device acceptance flow, and record evidence in the delivery workflow.
...
- Should the first validation run prove two real Android devices talking to each other, or is one Android device plus a scripted counterpart session sufficient for initial acceptance?
- Do we want to keep the long-term preferred validation path as `adb reverse` + SSH tunnel, or should a later change add a public HTTPS/WebSocket gateway for broader QA access?
