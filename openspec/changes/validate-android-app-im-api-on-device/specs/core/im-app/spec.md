...
### Requirement: Android app exercises live IM backend flows during device validation
The system SHALL let the Android app authenticate through the backend development session flow, hydrate conversations and message history from the live IM backend, and reconcile authenticated WebSocket events into visible chat state during physical-device validation runs.

#### Scenario: Device validation bootstraps live conversation data
- **WHEN** a tester starts an IM validation session on the Android app with a configured development user
...
#### Scenario: Device validation drives realtime send and receipt updates
- **WHEN** the tester sends a message from the Android app while the backend and counterpart session are online
...
#### Scenario: Device validation recovers after reconnect or relaunch
- **WHEN** the Android app loses its realtime connection or is relaunched during a validation session
...
#### Scenario: Device validation surfaces backend failures explicitly
- **WHEN** session issuance, bootstrap loading, history retrieval, or realtime connection setup fails on the Android device
...
#### Scenario: Operator updates IM validation endpoints on a physical device
- **WHEN** a tester enters or selects IM backend endpoint values and a development user identity on the Android device
