...
- **THEN** the repository provides the scripts, service shape, or documented commands needed to deploy that slice to the Ubuntu host, inspect logs or service status, confirm the backend health endpoint, and run a remote smoke test before the next slice begins
