...
The system SHALL make a successful tag build downloadable from GitHub, and it MUST publish the resulting APK as both a workflow artifact and a tag-bound GitHub Release asset.
...
- **THEN** the built APK is uploaded to the workflow run and attached to the GitHub Release associated with that tag

#### Scenario: Failed build does not leave a misleading published release
- **WHEN** the workflow fails before the APK build or signing/publish steps complete
- **THEN** GitHub does not present a completed release asset for that tag as if the APK had been produced successfully
