...
### Task 1.4 (companion-settings-and-safety-reframe): Extend `ImBackendModels.kt` so `CompanionTurnBlocked`/`CompanionTurnFailed` expose typed enum projections over wire keys. (commit `<pending>`)
