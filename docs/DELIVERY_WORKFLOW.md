...
### Task 2.3 (companion-settings-and-safety-reframe): Extend `ChatMessageRow`'s `Timeout` terminal to render `SafetyCopy.timeoutCopy` body, a primary Retry affordance, and a conditional "Switch preset" hint gated by the active preset's `maxReplyTokens` heuristic cap. (commit `<pending>`)
