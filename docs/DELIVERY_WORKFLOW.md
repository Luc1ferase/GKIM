...
### Task 2.4 (companion-settings-and-safety-reframe): Add `ChatFailureAndSafetyBubbleInstrumentationTest` on codex_api34 covering parsed companion_turn.failed / blocked / timeout events → correct copy + correct per-subtype actions. (commit `<pending>`)
