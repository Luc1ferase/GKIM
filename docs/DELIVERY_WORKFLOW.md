...
### Task 2.1 (companion-settings-and-safety-reframe): Extend `ChatMessageRow`'s `Blocked` terminal to render `BlockReasonCopy.localizedCopy`, "Compose a new message" CTA, and "Learn more" link (no retry). (commit `<pending>`)
