...
### Task 2.2 (companion-memory-and-preset): Add `LiveCompanionMemoryRepository.kt` that wires `DefaultCompanionMemoryRepository` to `ImBackendClient` HTTP endpoints with optimistic-update + rollback-on-failure semantics.
