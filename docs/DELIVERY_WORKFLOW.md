...
### Task 3.1 (companion-memory-and-preset): Add `CompanionPresetRepository.kt` + `DefaultCompanionPresetRepository` with sealed `CompanionPresetMutationResult` and three enforced invariants (built-in immutability, exactly-one-active, duplicate bilingual suffix). (commit `<pending>`)
