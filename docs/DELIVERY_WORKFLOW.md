...
### Task 3.2 (companion-settings-and-safety-reframe): Add the `Companion` section's memory-shortcut entry with a chooser listing recently active companions (active-first ordering) routing to the scoped memory panel. (commit `<pending>`)
