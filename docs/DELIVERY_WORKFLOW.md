...
### Task 3.4 (companion-settings-and-safety-reframe): Rewrite the `AIGC Image Provider` section caption so it scopes to image generation and disambiguates from companion chat, with no change to provider selection logic. (commit `<pending>`)
