...
### Task 6.1 (companion-memory-and-preset): Finalize `openspec/changes/companion-memory-and-preset/specs/im-backend/spec.md` covering memory + preset HTTP endpoints, persistence, summarization trigger, allocator, `prompt_budget_exceeded`, and idempotent seeding. (commit `<pending>`)
