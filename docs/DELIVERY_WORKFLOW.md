...
### Task §6.1 / §6.2 (companion-turn-character-prompt-context): Verification roll-up + archive — apply spec delta to `openspec/specs/llm-text-companion-chat/spec.md`, move change to `openspec/changes/archive/2026-04-26-companion-turn-character-prompt-context/`. (commit TBD)

- Verification:
  - `openspec archive companion-turn-character-prompt-context --yes` reported `Specs to update: llm-text-companion-chat: update / + 2 added / Specs updated successfully.` and confirmed `Change 'companion-turn-character-prompt-context' archived as '2026-04-26-companion-turn-character-prompt-context'`.
  - `openspec validate --specs --strict` (post-archive) — `16 passed, 0 failed`. The two ADDED Requirements (`Client passes resolved character prompt context with every companion-turn submit, edit, and regenerate-at request` + `LiveCompanionTurnRepository carries character prompt context through the retry path`) merge cleanly into `openspec/specs/llm-text-companion-chat/spec.md` after the existing tavern-experience-polish-era requirements.
  - Slice commit chain: §1.1 (`6edc786` / `0734662` SHA fill) → §2.1 (`d5b9fd9` / `b849987`) → §2.2 (`81b8645` / `0d0480b`) → §2.3 (`271e6ce` client + `5f4089a` paired backend mirror / `37a4341`) → §3.1 (`03283ad` / `46eb41d`) → §4.1+§4.2 (`8136b5d` / `8e95130`) → §5.1+§5.2 (`ed57bd4` / `2bd89cd`) → §6.1 evidence (`cd5b010`) → §6.2 archive (this commit).
- Review:
  - Score: `95/100`
  - Findings: `§6 closes the paired-PR client slice. The diff against feature/ai-companion-im is bounded: 1 new feature/chat/CharacterPromptContextResolver.kt (~40 lines) + 1 new feature/chat/CharacterPromptContextResolverTest.kt + 1 new data/remote/im/CompanionTurnSubmitRequestDtoSerializationTest.kt + 2 new contract/fixtures/companion-turns/submit-request-{with,without}-character-context.json + 1 modified data/remote/im/ImBackendModels.kt (one new DTO + three field additions) + 1 modified data/repository/CompanionTurnRepository.kt (interface signature + FailedCompanionSubmission field) + 1 modified data/repository/LiveCompanionTurnRepository.kt (three method threadings + retry replay) + 1 modified feature/chat/ChatRoute.kt (constructor + helper + three call-site updates + route lambda for regenerateFromHere) + 1 modified feature/chat/ChatViewModelTestStubs.kt (stubCompanionRosterRepository helper) + 5 modified feature/chat/ChatViewModel*Test.kt + 3 modified data/repository/LiveCompanionTurn*Test.kt. The §1-§5 design rationale is preserved verbatim in archive/ and the spec delta merges cleanly into llm-text-companion-chat (2 ADDED Requirements, 6 Scenarios — 4 covering submit/edit/regenerate-at parity + missing-card omission + missing-persona localized default + edit/regenerate-at carry-the-payload-identically; 2 covering retry replay + retry-without-captured-payload). The 5-point deduction reflects (a) the slice does NOT verify the on-disk fixtures byte-mirror via a CI script — the paired-PR convention is enforced manually via `git diff --no-index`. (b) the slice's end-to-end smoke against a real LLM-bridged backend is owned by the paired backend slice's §6.1; until that lands, the client wire shape is verified by unit tests + serialization tests against on-disk fixtures only.`
- Upload:
  - Commit: TBD
  - Branch: `feature/companion-turn-character-prompt-context`
  - Push: `origin/feature/companion-turn-character-prompt-context`
