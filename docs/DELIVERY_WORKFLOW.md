...
### Task 5.3 (companion-memory-and-preset): Add `BubblePinAction` helper (draft + dispatch) with `BubblePinActionTest` covering pin-from-user / pin-from-variant / pin surfaces-in-observer. (commit `<pending>`)
