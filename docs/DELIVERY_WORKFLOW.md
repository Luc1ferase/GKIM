...
### Task 3.3 (companion-settings-and-safety-reframe): Add the `Content & Safety` section's two items: a read-only "Acknowledgment status" row showing acceptance date or "Not accepted — read policy" routing to `ContentPolicy`, and a "Block reason verbosity" toggle row (default on) opening a detail screen with a Switch that persists through `PreferencesStore`. (commit `<pending>`)
