...
### Task 3.2 (companion-memory-and-preset): Add `LiveCompanionPresetRepository.kt` binding the default repository to `ImBackendClient` with parallel cold-start refresh (list + active-preset) and optimistic-rollback mutations. (commit `<pending>`)
