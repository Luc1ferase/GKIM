...
### Task 5.4 (companion-memory-and-preset): Add instrumentation `MemoryAndPresetIntegrationTest` on `codex_api34` covering memory-panel hydration + pin-create flow + three reset scopes + cancel confirmation. (commit `<pending>`)
