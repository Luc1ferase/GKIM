...
### Task 4.1 (companion-settings-and-safety-reframe): Add `ContentPolicyAcknowledgmentRoute` + `ContentPolicyAcknowledgmentViewModel` + bilingual `ContentPolicyCopy`; accept CTA calls backend + persists acknowledgment. (commit `<pending>`)
