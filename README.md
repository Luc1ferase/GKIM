...
Android 原生客户端 + 私有 Rust 实时后端 + AIGC 创作能力的 IM 项目，全程由 AI 编码代理驱动开发。
...
| **Rust 后端（私有源码）** | Rust · Axum · Tokio · PostgreSQL (SQLx) · Argon2 · Docker | HTTP + WebSocket 即时通信服务 |

客户端选择 Android 原生以保证 IM 场景对交互流畅度、状态管理和实时连接的要求；后端选择 Rust 以适配高并发、长连接和状态一致性的需求。公开仓库只保留客户端与交付资产，后端源码通过维护者私有路径保存。
...
- **酒馆角色** — 第三个主 tab 提供 tavern 式角色选择、预设角色卡、抽卡结果与已持有角色管理，并把激活角色带入陪伴对话
- **AIGC 创作** — 多提供商适配架构（腾讯混元 / 阿里通义 / 自定义 OpenAI 兼容端点），支持 text-to-image、image-to-image、video-to-video 三种模式
- **陪伴角色配置** — 保留 AI/模型配置能力，并逐步从创作工具流转向陪伴角色、记忆与人格控制
...
│       └── feature/  #   Welcome、Login、Chat、Contacts、Tavern、Settings 等
├── backend/          # 公共占位说明；后端源码与运维资产改为维护者私有保存
├── openspec/         # 规格驱动交付框架
│   ├── changes/      #   活跃 + 归档变更
...
**后端源码边界**：详见 [`backend/README.md`](backend/README.md)
...
- 推荐发布入口：`powershell -ExecutionPolicy Bypass -File .\scripts\release-android-tag.ps1 -TagName vMAJOR.MINOR.PATCH`

仓库内置的发布入口会先做 release preflight，再触发真正的 GitHub 发布：

- 拦截脏 worktree、未同步到远端的当前分支、以及已存在的版本 tag
- 先 push 当前分支，再创建并 push release tag
- 返回 GitHub Actions run URL、`gh run watch` 命令，以及最终 `gh release view` 检查命令
- GitHub workflow 会在 Step Summary 里写出 tag、预期 APK 名称和最终 publication state
...
详细的 Android 构建和本地验证说明见 [`android/README.md`](android/README.md)，release 操作步骤见 [`docs/android-tag-release-operations.md`](docs/android-tag-release-operations.md)。

## 测试

| 层级 | 工具 | 覆盖范围 |
|---|---|---|
| 私有 Rust 后端 | cargo test（在维护者私有后端 checkout 中运行） | HTTP API、WebSocket 网关、Schema 迁移集成测试 |
